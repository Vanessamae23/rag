...
def analyze_bert_scores():
    ...
    cleaned_texts = [item["Cleaned"] for item in data]
    uncleaned_texts = [item["Uncleaned"] for item in data]
    ...
    print(f"\nAverage Cleaned BERT Score F1: {cleaned_avg_f1:.4f}")
    print(f"Average Uncleaned BERT Score F1: {uncleaned_avg_f1:.4f}")
    print(f"Average Cleaned Cosine Similarity: {cleaned_avg_cosine:.4f}")
    print(f"Average Uncleaned Cosine Similarity: {uncleaned_avg_cosine:.4f}")
    ...
    for i, row in df.iterrows():
        if row["Cleaned_BERTScore_F1"] is not None and row["Uncleaned_BERTScore_F1"] is not None:
            ...
            plot_data.append({"Answer": answer_text, "Score": row["Cleaned_BERTScore_F1"], "Type": "Cleaned"})
            plot_data.append({"Answer": answer_text, "Score": row["Uncleaned_BERTScore_F1"], "Type": "Uncleaned"})
    
    plot_df = pd.DataFrame(plot_data)
    if not plot_df.empty:
        sns.barplot(x="Answer", y="Score", hue="Type", data=plot_df)
        plt.ylabel("BERT Score F1")
        plt.title("Comparison of BERT Score F1: Cleaned vs. Uncleaned")
    ...
    for i, row in df.iterrows():
        if row["Cleaned_Cosine_Similarity"] is not None and row["Uncleaned_Cosine_Similarity"] is not None:
            answer_text = row["Answer"][:20] + "..." if row["Answer"] else "N/A"
            plot_data.append({"Answer": answer_text, "Similarity": row["Cleaned_Cosine_Similarity"], "Type": "Cleaned"})
            plot_data.append({"Answer": answer_text, "Similarity": row["Uncleaned_Cosine_Similarity"], "Type": "Uncleaned"})
    
    plot_df = pd.DataFrame(plot_data)
    if not plot_df.empty:
        sns.barplot(x="Answer", y="Similarity", hue="Type", data=plot_df)
        plt.ylabel("Cosine Similarity")
        plt.title("Comparison of Cosine Similarity: Cleaned vs. Uncleaned")
        plt.xticks(rotation=45)
        plt.legend(title="Text Type")
    
    plt.tight_layout()
    plt.savefig("bert_score_main.png")
...
# Save results to CSV
results_df.to_csv("bert_score_main.csv", index=False)
